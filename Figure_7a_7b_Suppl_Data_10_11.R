...
df_sleuth <- data.table(read_excel("Supplementary Data.xlsx", sheet = 4, skip = 1))
...
df_ase <- subset(data.table(read_excel("Supplementary Data.xlsx", sheet = 8, skip = 1, col_types = c("text", "numeric", "text", "text", "text", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "logical", "logical"))), contig == "chrX" & sample == "F3" & keep == T)
...
# make suppl table 13
Suppl_Data_10 <- smash2_short_melt
colnames(Suppl_Data_10) <- c("gene", "ASE_F3", "Methylation_category", "log2FC", "log2FC") 

# add classification column, exporting which XCI call we made per gene.
Suppl_Data_10$category <- "LOL"
Suppl_Data_10[!is.na(Suppl_Data_10$ASE_F3),]$category <- Suppl_Data_10[!is.na(Suppl_Data_10$ASE_F3),]$ASE_F3
Suppl_Data_10[Suppl_Data_10$gene == "XIST"]$category <- "Escape"
Suppl_Data_10[is.na(Suppl_Data_10$ASE_F3) & Suppl_Data_10$Methylation_category == 0.25 & Suppl_Data_10$log2FC > 0.2,]$category <- "high_confidence_escapee"
Suppl_Data_10[is.na(Suppl_Data_10$ASE_F3) & Suppl_Data_10$Methylation_category == 0.25 & Suppl_Data_10$log2FC < 0.2 & Suppl_Data_10$log2FC > 0,]$category <- "low_confidence_escapee"

Suppl_Data_10[is.na(Suppl_Data_10$ASE_F3) & Suppl_Data_10$Methylation_category != 0.25 & Suppl_Data_10$log2FC < 0.2,]$category <- "Inactive"

Suppl_Data_10[is.na(Suppl_Data_10$ASE_F3) & 
                     (
                         (Suppl_Data_10$Methylation_category == 0.25 & Suppl_Data_10$log2FC < 0) 
                         |
                         (Suppl_Data_10$Methylation_category != 0.25 & Suppl_Data_10$log2FC > 0.2)
...
write.table(dplyr::select(Suppl_Data_10, -log2FC), file = "05_reports/Suppl_Data_10.tsv", quote = F, col.names = T, row.names = F, sep = "\t")


# make alluvial plot
class_for_alluvium <- Suppl_Data_10

# read in thymocyte TPMs
df_TPM <- data.table(read_excel("Supplementary Data.xlsx", sheet = 2, skip = 1, col_types = c("text",rep(paste0("numeric"), 46))))
...
# make suppl table 14
Suppl_Data_11 <- smashf_fin_filt

Suppl_Data_11[Suppl_Data_11$axes == "Tukiainen",]$axes <- "previous assessment"
Suppl_Data_11[Suppl_Data_11$axes == "Gylemo",]$axes <- "thymocytes"

Suppl_Data_11 <- merge(Suppl_Data_11[Suppl_Data_11$axes == "thymocytes",c("gene","category")], Suppl_Data_11[Suppl_Data_11$axes == "previous assessment",c("gene","category")], by = "gene")

colnames(Suppl_Data_11) <- c("gene", "thymocytes_classification", "previous_assessment")

Suppl_Data_11$comparison <- ifelse(Suppl_Data_11$thymocytes_classification == Suppl_Data_11$previous_assessment, yes = "same", no = "differs")

Suppl_Data_11[Suppl_Data_11$thymocytes_classification == "high_confidence_escapee" & Suppl_Data_11$previous_assessment == "Escape"]$comparison <- "same"
Suppl_Data_11[Suppl_Data_11$thymocytes_classification == "low_confidence_escapee" & Suppl_Data_11$previous_assessment == "Escape"]$comparison <- "same"

Suppl_Data_11 %>% dplyr::group_by(comparison) %>% dplyr::count()

write.table(dplyr::select(Suppl_Data_11, gene, previous_assessment, thymocytes_classification, comparison), file = "05_reports/Suppl_Data_11.tsv", quote = F, col.names = T, row.names = F, sep = "\t")
